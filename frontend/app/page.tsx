...
// ── session persistence (survives page reload, cleared on explicit leave) ──
...
function saveRoomSession(data: RoomSession): void {
  sessionStorage.setItem('nk_room_secret', data.roomSecret);
  sessionStorage.setItem('nk_room_id', data.roomId);
  sessionStorage.setItem('nk_invite_token', data.inviteToken);
  sessionStorage.setItem('nk_expected_fp', data.expectedFingerprint);
  sessionStorage.setItem('nk_username', data.username);
}

function loadRoomSession(): RoomSession | null {
  const roomSecret = sessionStorage.getItem('nk_room_secret');
  const roomId = sessionStorage.getItem('nk_room_id');
  const inviteToken = sessionStorage.getItem('nk_invite_token');
  if (!roomSecret || !roomId || !inviteToken) return null;
  return {
    roomSecret,
    roomId,
    inviteToken,
    expectedFingerprint: sessionStorage.getItem('nk_expected_fp') || '',
    username: sessionStorage.getItem('nk_username') || 'Anonymous',
  };
}

function clearRoomSession(): void {
  sessionStorage.removeItem('nk_room_secret');
  sessionStorage.removeItem('nk_room_id');
  sessionStorage.removeItem('nk_invite_token');
  sessionStorage.removeItem('nk_expected_fp');
  sessionStorage.removeItem('nk_username');
}
...
export default function HomePage() {
  ...
  return sessionStorage.getItem('nk_username') || 'Anonymous';
});
  ...
  const latestInviteTokenRef = useRef('');
  const usernameRef = useRef(username);
  const peerSessionManagerRef = useRef(new PeerSessionManager());
  ...
  const handleKeyExchangeFromServerRef = useRef<(payload: any) => void>((_) => {});
  ...
  useEffect(() => {
    ...
    const init = async () => {
      ...
      peerSessionManagerRef.current.setSendKeyExchange(
        () => {
          const sm = sessionManagerRef.current;
          if (!sm) return;
          wsClient.sendKeyExchange({
            room_id: activeRoomIdRef.current,
            sender_fingerprint: sm.fingerprint,
            sender_public_key: publicKeyBase64(),
            sender_username: usernameRef.current || 'Anonymous',
          });
        },
      );

      // Subscribe to server events — registered before WS connects so no events are dropped
      unsubMessage = wsClient.onMessage((event, payload) => {
        switch (event as RelayEvent) {
          ...
          case 'key:exchange': {
            handleKeyExchangeFromServerRef.current(payload);
            break;
          }
          ...
          case 'client:joined': {
            if (payload?.fingerprint) {
              ...
              peerSessionManagerRef.current.onPeerJoined(
                payload.fingerprint,
          ...
          case 'client:left': {
            if (payload?.fingerprint) {
              peerSessionManagerRef.current.onPeerLeft(payload.fingerprint);
      ...
      unsubStatus = wsClient.onStatus((s: string) => {
        if (s === 'open') {
          ...
          // Check sessionStorage for a room to rejoin after reload
          if (!activeRoomIdRef.current) {
            const session = loadRoomSession();
            if (session) {
              activeRoomIdRef.current = session.roomId;
              latestInviteTokenRef.current = session.inviteToken;
              setActiveRoomId(session.roomId);
              setActiveRoomSecret(session.roomSecret);
              if (session.expectedFingerprint) setExpectedFingerprint(session.expectedFingerprint);
              setUsername(session.username);
              setShowChatInterface(true);
              peerSessionManagerRef.current.onRoomJoined();
              wsClient.createRoom(
                session.roomId,
                session.inviteToken,
                sm.fingerprint,
          ...
          if (activeRoomIdRef.current) {
            ...
            wsClient.createRoom(
              ...
              latestInviteTokenRef.current || 'reconnect',
              sm.fingerprint,
              usernameRef.current,
            );
          }
        ...
        if (s === 'close') {
          setConnected(false);
          peerSessionManagerRef.current.onDisconnected();
          setStatus('Disconnected from server');
        }
        if (s === 'error') setStatus('Connection error');
      });

      // Auto-connect — triggers onStatus('open') which handles
      // both manual room creation/join and auto-rejoin from sessionStorage.
  ...
  // Skip if sessionStorage has a room session (reconnect after reload)
  useEffect(() => {
  ...
  const handleKeyExchangeFromServer = useCallback(
    (payload: any) => {
      ...
      // Feed to state machine — onKeyReceived decodes base64 internally
      const publicKey = peerSessionManagerRef.current.onKeyReceived(
        payload.sender_fingerprint,
        payload.sender_public_key,
        payload.sender_username || 'Anonymous',
      );
      if (!publicKey) {
        console.error('Failed to decode public key from peer');
        return;
      }

      try {
        sm.establishSharedSecret(publicKey, payload.sender_fingerprint);
        peerSessionManagerRef.current.markSharedSecretReady(payload.sender_fingerprint);
        setPeers((prev) => {
  ...
  handleKeyExchangeFromServerRef.current = handleKeyExchangeFromServer;
  ...
  const handleJoinRoom = async (secret?: string, token?: string) => {
    ...
    try {
      ...
      if (!sm) { setStatus('Session not initialized'); return; }
      if (!ws) { setStatus('Not connected'); return; }
      ws.joinRoom(roomId, inviteToken, sm.fingerprint, username);
      setActiveRoomSecret(roomSecret);
      setActiveRoomId(roomId);
      latestInviteTokenRef.current = inviteToken;
      activeRoomIdRef.current = roomId;
  ...
  const handleSendMessage = async () => {
    ...
    const recipients = peers.filter((p) => peerSessionManagerRef.current.isReady(p.fingerprint));
